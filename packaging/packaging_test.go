...
// ID does not. That only works while the ID in internal/platform/register_windows.go and
...
func TestWindowsDocumentIconIDMatchesTheRegistryValue(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join("windows", "winres.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for id, want := range map[string]string{"#1": "htmlclay.ico", "#2": "doc.ico"} {
		...
		if _, err := os.Stat(filepath.Join("windows", file)); err != nil {
	...
	src, err := os.ReadFile(filepath.Join("..", "internal", "platform", "register_windows.go"))
	if err != nil {
		t.Fatal(err)
	}
	m := regexp.MustCompile(`docIconIndex = "(,-\d+)"`).FindSubmatch(src)
	if m == nil {
		t.Fatal("internal/platform/register_windows.go no longer declares docIconIndex as a negative icon index")
	}
...
func TestLinuxDocumentIconIsNamedAfterTheMimeType(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join("linux", "htmlclay-mime.xml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	installer, err := os.ReadFile(filepath.Join("linux", "install-icon.sh"))
	if err != nil {
		t.Fatal(err)
	}
	for _, ext := range []string{".png", ".svg"} {
		file := iconName + ext
		if _, err := os.Stat(filepath.Join("linux", file)); err != nil {
