...
#   internal/tray/icon.png                colored grin, used by systray on Linux
#   internal/tray/icon.ico                same grin as ICO, required by systray on Windows
#   internal/tray/icon-template.png       black grin silhouette, macOS menu-bar template
#   packaging/macos/htmlclay.icns     app icon  (CFBundleIconFile = htmlclay)
#   packaging/macos/doc.icns          document icon (CFBundleTypeIconFile = doc)
#   packaging/linux/htmlclay.png      app icon, 256px (install-icon.sh)
#   packaging/linux/htmlclay.svg      app icon, scalable
#   packaging/windows/htmlclay.ico    app icon, multi-size
#   packaging/windows/doc.ico         document icon, embedded as resource 2 (winres.json)
#   packaging/linux/application-x-htmlclay.png   document icon, 256px, mimetypes/
#   packaging/linux/application-x-htmlclay.svg   document icon, scalable, mimetypes/
#
...
echo "  -> packaging/macos/htmlclay.icns"
...
if command -v iconutil >/dev/null 2>&1; then
  iconutil -c icns "$ICONSET" -o "$ROOT_DIR/packaging/macos/htmlclay.icns"
else
  echo "  (iconutil missing; using ImageMagick)"
  "$MAGICK" "$BUILD/app-16.png" "$BUILD/app-32.png" "$BUILD/app-128.png" \
    "$BUILD/app-256.png" "$BUILD/app-512.png" "$BUILD/app-1024.png" "$ROOT_DIR/packaging/macos/htmlclay.icns"
fi

echo "  -> packaging/windows/htmlclay.ico"
render "$BUILD/app.svg" 48 "$BUILD/app-48.png"   # 48 is an ICO size but not an ICNS one
write_ico "$ROOT_DIR/packaging/windows/htmlclay.ico" "$BUILD" app

echo "  -> packaging/linux/htmlclay.png (256) + htmlclay.svg"
cp "$BUILD/app-256.png" "$ROOT_DIR/packaging/linux/htmlclay.png"
cp "$BUILD/app.svg"     "$ROOT_DIR/packaging/linux/htmlclay.svg"

echo "Rendering document icon -> packaging/macos/doc.icns"
...
if command -v iconutil >/dev/null 2>&1; then
  iconutil -c icns "$DOCSET" -o "$ROOT_DIR/packaging/macos/doc.icns"
else
  "$MAGICK" "$BUILD/doc-16.png" "$BUILD/doc-32.png" "$BUILD/doc-128.png" \
    "$BUILD/doc-256.png" "$BUILD/doc-512.png" "$BUILD/doc-1024.png" "$ROOT_DIR/packaging/macos/doc.icns"
fi

echo "  -> packaging/windows/doc.ico"
render "$BUILD/doc.svg" 48 "$BUILD/doc-48.png"
write_ico "$ROOT_DIR/packaging/windows/doc.ico" "$BUILD" doc

echo "  -> packaging/linux/application-x-htmlclay.png (256) + .svg"
cp "$BUILD/doc-256.png" "$ROOT_DIR/packaging/linux/application-x-htmlclay.png"
cp "$BUILD/doc.svg"     "$ROOT_DIR/packaging/linux/application-x-htmlclay.svg"

echo "Rendering tray icons -> internal/tray/icon.png + internal/tray/icon.ico + internal/tray/icon-template.png"
...
"$MAGICK" "$BUILD/grin.png" -trim +repage -background none -gravity center -extent 231x231 \
  -resize 128x128 "$ROOT_DIR/internal/tray/icon.png"
...
# packaging/icons/make-tray-ico.ps1 rebuilds this on a Windows box, where this script's
# rsvg-convert/ImageMagick dependencies usually are not installed.
"$MAGICK" "$ROOT_DIR/internal/tray/icon.png" -define icon:auto-resize=48,32,16 "$ROOT_DIR/internal/tray/icon.ico"
...
"$MAGICK" "$BUILD/tpl.png" -trim +repage -background none -gravity center -extent 235x235 \
  -resize 64x64 "$ROOT_DIR/internal/tray/icon-template.png"

echo
echo "Done. Generated:"
for f in internal/tray/icon.png internal/tray/icon.ico internal/tray/icon-template.png \
         packaging/macos/htmlclay.icns packaging/macos/doc.icns \
         packaging/linux/htmlclay.png packaging/linux/htmlclay.svg \
         packaging/linux/application-x-htmlclay.png packaging/linux/application-x-htmlclay.svg \
         packaging/windows/htmlclay.ico packaging/windows/doc.ico; do
