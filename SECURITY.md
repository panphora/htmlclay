...
When two trusted folders are nested, the outer one owns the whole tree and serves it on one address. Untrusting the outer one hands the inner folder back its own address.
...
5. **Hard links.** Someone who can already create hard links inside your home folder could link HTML Clay's own files into a folder you then trust. That takes an attacker who is already on your machine and can read those files directly, so it adds no exposure they did not already have.
6. **Inside a folder you allowed or trusted, a page can tell which files exist.** That comes with read access and is not separately preventable.
